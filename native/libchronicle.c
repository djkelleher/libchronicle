...
void handle_dirlist_uint64(char *buf, int sz, uint64_t data, wirecallbacks_t *cbs)
{
    ...
    {
        if (debug)
            printf("  v5 roll_length set to %lx\n", data);
        queue->roll_length = data;
    }
    else if (strncmp(buf, "epoch", sz) == 0)
    {
        if (debug)
            printf("  v5 roll_epoch set to %lx\n", data);
...
void handle_qf_uint64(char *buf, int sz, uint64_t data, wirecallbacks_t *cbs)
{
    ...
    {
        if (debug)
            printf(" v4 roll_length set to %lx\n", data);
        queue->roll_length = data;
    }
    ...
    {
        if (debug)
            printf(" v4 roll_epoch set to %lx\n", data);
        queue->roll_epoch = data;
    }
}
...
tailstate_t chronicle_peek_queue_tailer_r(queue_t *queue, tailer_t *tailer)
{
    ...
    while (1)
    {
        ...
        if (basep != basep_old)
        {
            ...
            if (debug)
                printf("shmipc:  parser moved shm %p to %p, file %" PRIu64 " -> %" PRIu64 ", index %" PRIu64 " to %" PRIu64 "\n", basep_old, basep, tailer->qf_tip, new_tip, tailer->qf_index, index);
